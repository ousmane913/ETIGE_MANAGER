...
export default function Show({ project }: any) {
    ...
    return <AppLayout><div className="mb-8 flex items-start justify-between"><div><Link href="/projets/" className="text-sm font-semibold text-amber-700">← Projets</Link><p className="mt-3 text-xs font-bold text-amber-700">{project.reference}</p><h2 className="text-3xl font-bold">{project.name}</h2><p className="mt-1 text-slate-500">{project.client} · {project.address}</p></div><Status value={project.status} /></div><section className="grid gap-5 lg:grid-cols-3"><div className="card lg:col-span-2"><h3 className="font-bold">Workflow opérationnel</h3><p className="mb-5 mt-1 text-sm text-slate-500">Chaque étape s'ouvre lorsque les prérequis sont remplis.</p><div className="grid gap-3"><Step title="1. Survey" done={!!survey?.validated} href={`/projets/${project.id}/survey/`} text={survey?.validated ? `Visite validée le ${survey.visitDate}` : 'Visite technique et constats.'} /><Step title="2. Devis" done={quote?.status === 'APPROVED'} href={`/projets/${project.id}/devis/`} text={quote ? `${quote.number} — ${quote.status === 'APPROVED' ? 'validé' : 'à valider'}` : 'Chiffrage après validation du Survey.'} /><Step title="3. Achats" done={purchaseDone} href={`/projets/${project.id}/achats/`} text={purchaseDone ? 'Au moins une commande réceptionnée.' : 'Commandes fournisseurs après devis validé.'} /><Step title="4. Chantier" done={site?.status === 'COMPLETED'} href={`/projets/${project.id}/chantier/`} text={site ? `${site.progress}% d'avancement` : 'Exécution après réception des achats.'} /><Step title="5. Rapport / clôture" done={!!report} href={`/projets/${project.id}/cloture/`} text={report ? `Livré le ${report.deliveredOn}` : 'Bilan après la fin du chantier.'} /></div></div><aside className="space-y-5">
        ...
        {hasFinancialData && <div className="card"><h3 className="font-bold">Bilan Financier</h3><dl className="mt-4 space-y-3 text-sm">
            ...
            {project.profit && <div className="flex justify-between border-t border-slate-200 pt-3"><dt className="text-slate-500 font-medium">Bénéfice</dt><dd className={`font-bold ${Number(project.profit) >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{Number(project.profit).toLocaleString('fr-FR')} FCFA</dd></div>}
        </dl></div>}

        {/* Dépenses : liste visible par tous, lien ajout réservé DG/DT */}
        <div className="card"><div className="flex items-center justify-between"><h3 className="font-bold">Dépenses hors-devis</h3>{isManagement && <Link href={`/projets/${project.id}/depense/`} className="text-xs font-bold text-amber-700">+ Ajouter</Link>}</div>{expenses.length > 0 ? <ul className="mt-4 space-y-3">{expenses.map((exp: any, i: number) => <li key={i} className="flex justify-between text-sm"><span className="text-slate-600">{exp.description}</span><span className="font-semibold">{Number(exp.amount).toLocaleString('fr-FR')} FCFA</span></li>)}</ul> : <p className="mt-4 text-sm text-slate-500">Aucune dépense enregistrée.</p>}</div>

    </aside></section><section className="mt-5 grid gap-5 lg:grid-cols-2"><PhotoGallery title="Photos du Survey" photos={surveyPhotos} /><PhotoGallery title="Photos du résultat final" photos={closurePhotos} /></section></AppLayout>
}
